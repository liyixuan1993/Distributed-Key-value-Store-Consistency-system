...
/*
For LinearReplica, the underlying multicaster is TotalOrderingMulticaster so that the real time
order is preserved
Event: same in eventualReplica,difference is it does not have Value,since we do not need timeStamp
in get operation.
onRequest: if get,we just get the value from the replica.if put,we should multicast that msg to
other replicas,and wait to receive all the response from other replicas.
