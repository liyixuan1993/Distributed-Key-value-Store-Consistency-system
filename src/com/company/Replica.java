...
/*
In order to operate variables,we need to create Map<String, Value> dataMap,Value contains var's 
value and timestamp.timestamp is used in eventual consistency for last-writer-wins algorithm.

In order to response to client, we need Map<String, ClientNode> clientMap,String is client's id 
and ClientNode contains outputStream and privateData.privateData is actually Event,which stores 
Value and count

dumpData: iterate dataMap and print on  the screen.

onClientData: put client id and outputStream into clientMap,and go to onRequest function. 
*/
